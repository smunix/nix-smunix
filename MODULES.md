...
| VPS host manifest | `hosts/vps-73025e99/default.nix` | Selects the capabilities enabled for the remote OVH Cloud VPS `vps-73025e99` (Caddy, Hodari Accounting, dynamic MOTD, passwordless sudo). |
| VPS hardware | `hosts/vps-73025e99/hardware.nix` | Declares QEMU/KVM virtualized hardware modules for the OVH VPS instance. |
| VPS Disko layout | `hosts/vps-73025e99/disko.nix` | Declares declarative GPT partitioning for `/dev/sda` with BIOS boot (`EF02`), ESP (`EF00`), swap with discard, and ext4 root. |
...
| Zoom | `modules.desktop.socials.zoom.enable` | Installs Zoom video conferencing through the integrated Home Manager profile. |
...
| Module | Option | Responsibility |
|---|---|---|
| AI client selector | `modules.ai.enable`, `.clients` | Installs one or more typed AI coding-client choices through one host-level interface. `"kimi"` maps to `pkgs.kimi-code`, while `"antigravity"` maps to `pkgs.google-antigravity-cli`; the deprecated singular `.client` compatibility option overrides the list when explicitly set. |
...
| Google Antigravity CLI | `modules.ai.clients = ["antigravity"]` | Installs the reproducibly pinned official Linux `agy` binary with its self-updater disabled for immutable Nix operation. Authentication uses the user's Secret Service/browser or SSH authorization flow at runtime; no Google credential is stored by Nix. |
...
| Dioxus | `modules.develop.dioxus.enable`, `.desktop.*`, `.developmentServer.*`, `.developmentServer.caddy.*`, `.web.*`, `.android.*` | Requires the shared Rust module; installs the reproducibly pinned Dioxus 0.8.0-alpha.1 `dx` CLI, Linux GTK3/WebKitGTK/DBus desktop dependencies with deterministic closure-derived GLib, GIO, GObject, Pango, ATK, Cairo, GDK Pixbuf, pkg-config, data, GIO-module, and runtime paths, plus a configurable libclang package, comprehensive runtime and development `LIBRARY_PATH`, and focused Dioxus patch that injects every native search directory into both hot-patching fat-link commands; configurable `dioxus-serve` and compatibility `dioxus-serve-lan` helpers; optional user-owned Caddy HTTPS reverse proxying with dynamic interface-address discovery, NetworkManager-triggered start/restart/stop, internal-CA or fixed-host public-ACME TLS, security headers, compression, a root-owned/group-restricted low-port capability wrapper, and interface-scoped firewall policy. The user unit avoids namespace-based filesystem hardening that would isolate its file capability from the host network namespace. Web support provides declarative `wasm32-unknown-unknown`, configurable exact `wasm-bindgen-cli`, and Binaryen `wasm-opt`; Android support provides Android Studio plus a declarative API-35 SDK, Build Tools 35.0.0, NDK 27.2.12479018, CMake 3.22.1, emulator, Google APIs x86_64 image, Java 17, environment variables, KVM access, and the four requested Rust Android targets. |
...
| Hodari Accounting | `modules.services.hodari-accounting.enable`, `.port`, `.environmentFile`, `.openFirewall` | Provisions a hardened systemd service for the Dioxus-based Hodari Accounting & Invoicing web server (`hodari-server`) on loopback port 8080 with `DynamicUser`, `ProtectSystem=strict`, and private `/tmp`. Designed to run behind a Caddy reverse proxy. |
| Dynamic MOTD | `modules.services.motd.enable`, `.sshOnly`, `.networkInterface`, `.filesystems`, `.services` | Declaratively configures `rust-motd` with a KDL specification and hooks it into `interactiveShellInit` (for Bash) and Home Manager (for Nushell), displaying system generation, load averages, memory/swap, disk usage bars, IP addresses, and systemd service statuses with subshell guards. Supports `sshOnly` to toggle between SSH-only or local terminal displays. |
...
| Custom package set | `pkgs/default.nix`, `pkgs/google-antigravity-cli.nix`, `pkgs/rust-toolchain-policy.nix` | Builds the Rust-based `aya-tool`, Dioxus CLI 0.8.0-alpha.1, and wasm-bindgen CLI 0.2.128 packages with the same stable Rust 1.98.1 policy selected by `smunix`; pins the official Google Antigravity CLI 1.2.6 Linux binaries by architecture and published SHA-512 digest; and exports them with the exact prebuilt `bpf-linker` through the additions overlay. Per-system outputs additionally expose the Aya/eBPF NixOS VM and `ebpf-vm` launcher on x86_64 Linux. |

## Feature selection

The `smunix` manifest enables Plasma and the Niri/Noctalia desktop, NetworkManager, PipeWire, CUPS printing with Avahi/DNS-SD network discovery and automatic driverless queues, TLP with UPower telemetry, suspend-on-lid-close behavior, NVIDIA PRIME sync with disabled NVIDIA power management and host-specific PCI IDs, an ASUS DP-5 portrait-left/eDP-1 HiDPI-right layout generated for the pinned unstable Niri package with all named workspaces assigned to DP-5 and `Mod+O` focused-window monitor movement, automatic Tail 2 UVC detection and DJI Mic 3 PipeWire routing, the search and system utility groups, the language toolchains including Typst and centrally configured stable Rust 1.98.1 shared with Aya/eBPF and Dioxus, Dioxus desktop and web support with a loopback-only server on TCP 8080 behind a user-owned Caddy service that follows the current `wlp0s20f3` IPv4 address, Android Studio/SDK/NDK/emulator integration with `wasm32-unknown-unknown`, exact wasm-bindgen CLI 0.2.128, and four Android Rust targets, Aya’s exact bpf-linker, and isolated `ebpf-vm` laboratory, Kimi Code and Google Antigravity CLI through the multi-client AI module, Nushell, Starship, Zellij, Ghostty as the default terminal, WezTerm as an alternative, Brave, Firefox, OBS Studio with NDI and a V4L2 virtual camera, Discord, Signal, Zoom, Git, Jujutsu, Helix, Vim, Zed, the compact-font policy at a 20 percent reduction, automatic Plasma and Noctalia screen locking, Niri-only Gammastep at 6500 K by day and 3500 K after sunset using GeoClue automatic location with a bounded J0N 1P0 fallback, multi-finger enrollment and fingerprint authentication, YubiKey U2F authentication with password fallback, systemd-initrd FIDO2 unlocking for encrypted root and swap, the dynamic `rust-motd` system banner on interactive terminal and login sessions, and the shared Home Manager base/package profiles. For this service, only Caddy's TCP 80/443 and UDP 443 are permitted on that Wi-Fi interface; raw TCP 8080 remains closed. Niri additionally supplies XTerm, Okular, Evince, and Dolphin for its routed workspaces. The separate TDF, Zathura, and MPV viewer modules are enabled for terminal and graphical document or media viewing. An Xpdf routing rule is present, but the pinned insecure Xpdf package is intentionally not installed. The standalone Waybar module remains reusable but is not selected because Noctalia owns Niri’s bar.

The `vps-73025e99` manifest defines the remote OVH Cloud server running on a virtualized QEMU platform with hybrid UEFI/BIOS GRUB, Disko partitioning, DHCP IPv4, static IPv6 (`2607:5300:205:200::bc53/64`), and firewall access on ports 22, 80, and 443. It activates passwordless sudo for administration, decrypts age secrets using the host's existing SSH Ed25519 private key via `sops-nix`, configures GitLab deploy key authentication, runs the Dioxus-based Hodari Accounting server on loopback port 8080, provisions Caddy for automatic Let's Encrypt TLS termination at `accounting.hodari.ca` with permanent redirection from the default OVH hostname, and executes the dynamic `rust-motd` system dashboard on interactive SSH logins.
