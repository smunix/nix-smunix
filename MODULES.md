...
| Niri Gammastep | `modules.desktop.niri.gammastep.*` | Selects either validated manual coordinates or GeoClue automatic location, then fades between configurable day and night color temperatures through the Wayland adjustment method. Gammastep and the optional GeoClue agent are wanted by, ordered after, and stopped with `niri.service`; Niri desktop-environment conditions prevent both from running under Plasma. |
