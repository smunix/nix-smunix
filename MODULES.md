...
| Dioxus | `modules.develop.dioxus.enable`, `.desktop.*`, `.developmentServer.*`, `.web.*`, `.android.*` | Requires the shared Rust module; installs the reproducibly pinned Dioxus 0.8.0-alpha.1 `dx` CLI, Linux GTK3/WebKitGTK/DBus desktop dependencies with deterministic closure-derived GLib, GIO, GObject, Pango, ATK, Cairo, GDK Pixbuf, pkg-config, data, GIO-module, and runtime paths, plus a configurable libclang package, comprehensive runtime and development `LIBRARY_PATH`, and focused Dioxus patch that injects every native search directory into both hot-patching fat-link commands; a configurable `dioxus-serve-lan` helper with matching NixOS TCP firewall policy; declarative `wasm32-unknown-unknown` web support with configurable exact `wasm-bindgen-cli` selection and Binaryen `wasm-opt`, or Android Studio plus a declarative API-35 SDK, Build Tools 35.0.0, NDK 27.2.12479018, CMake 3.22.1, emulator, Google APIs x86_64 image, Java 17, environment variables, KVM access, and the four requested Rust Android targets. |
