...
| Niri and Noctalia | `modules.desktop.niri.enable` | Adds an SDDM-selectable Niri session with configurable Noctalia idle locking, screen-off delay, lock-before-suspend behavior, and Niri-only Gammastep solar scheduling; seven persistent named workspaces; application routing; a full-width `dumpster` fallback; widescreen proportions; and tabbed columns. It also owns the compositor, Xwayland Satellite, portals, keyring, desktop helpers, fonts, icons, cursor, and managed KDL/TOML configuration. |
| Niri Gammastep | `modules.desktop.niri.gammastep.*` | Calculates sunrise and sunset from validated coordinates, fades between configurable day and night color temperatures through the Wayland adjustment method, and is wanted by, ordered after, and stopped with `niri.service`. A Niri desktop-environment condition prevents it from running under Plasma. |
