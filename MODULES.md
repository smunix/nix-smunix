...
| Rust | `modules.develop.rust.enable`, `.nightlyVersion`, `.targets`, `.toolchain` | Resolves one configurable rust-overlay nightly for the host, including Cargo, rustc, rustfmt, Clippy, rust-analyzer, `rust-src`, and any declaratively selected compilation targets; the read-only `toolchain` package is shared with dependent modules. |
| Dioxus | `modules.develop.dioxus.enable`, `.desktop.enable`, `.web.*`, `.android.*` | Requires the shared Rust module; installs the reproducibly pinned Dioxus 0.8.0-alpha.1 `dx` CLI, Linux WebKitGTK and DBus desktop dependencies, declarative `wasm32-unknown-unknown` web support with Binaryen `wasm-opt`, or Android Studio plus a declarative API-35 SDK, Build Tools 35.0.0, NDK 27.2.12479018, CMake 3.22.1, emulator, Google APIs x86_64 image, Java 17, environment variables, KVM access, and the four requested Rust Android targets. |
| Aya/eBPF | `modules.develop.aya.enable`, `.vm.enable` | Requires the Rust module and reuses its configured nightly and cargo-generate through `aya-cargo` and `aya-rustc`; also provides pinned `aya-tool`, exact `bpf-linker` 0.11.1, bpftool, bpftrace, pahole, LLVM, tcpdump, and KVM access. The same `aya-tool` and bpftrace packages are installed inside the VM. The optional `ebpf-vm` command launches an isolated BPF/BTF/BPF-LSM NixOS laboratory, mounts its required `--shared-directory` path at `/host`, and forwards host loopback port 2222 to dev-only guest SSH, automatically authorizes a selected host public key, retains `dev` password recovery, and denies root SSH. |
