...
| Dioxus | `modules.develop.dioxus.enable`, `.desktop.enable`, `.web.*`, `.android.*` | Requires the shared Rust module; installs the reproducibly pinned Dioxus 0.8.0-alpha.1 `dx` CLI, Linux WebKitGTK and DBus desktop dependencies, declarative `wasm32-unknown-unknown` web support with configurable exact `wasm-bindgen-cli` selection and Binaryen `wasm-opt`, or Android Studio plus a declarative API-35 SDK, Build Tools 35.0.0, NDK 27.2.12479018, CMake 3.22.1, emulator, Google APIs x86_64 image, Java 17, environment variables, KVM access, and the four requested Rust Android targets. |
...
| Custom package set | `pkgs/default.nix` | Exports the pinned Aya `aya-tool`, exact `bpf-linker`, Dioxus CLI 0.8.0-alpha.1, and wasm-bindgen CLI 0.2.128 packages through the additions overlay. Per-system outputs additionally expose the Aya/eBPF NixOS VM and `ebpf-vm` launcher on x86_64 Linux. |
