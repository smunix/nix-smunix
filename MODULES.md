...
| Niri and Noctalia | `modules.desktop.niri.enable` | Adds an SDDM-selectable Niri session with configurable Noctalia idle locking, screen-off delay, lock-before-suspend behavior, and Niri-only Gammastep solar scheduling; seven persistent named workspaces; application routing; a full-width `dumpster` fallback; widescreen proportions; tabbed columns; and a DP-5 portrait-left/eDP-1 HiDPI-right output layout with full-width portrait columns. It also owns the compositor, Xwayland Satellite, portals, keyring, desktop helpers, fonts, icons, cursor, and managed KDL/TOML configuration. |
