...
| OBS Studio | `modules.programs.obs.enable`, `.ndi.enable`, `.virtualCamera.enable` | Installs wrapped OBS Studio, optionally adds the `obs-ndi` compatibility package backed by DistroAV and the unfree NDI 6 SDK, configures the V4L2 loopback virtual camera, and grants the primary user `video` and `render` access. |
