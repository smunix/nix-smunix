...
| Dynamic MOTD | `modules.services.motd.enable`, `.networkInterface`, `.filesystems`, `.services` | Declaratively configures `rust-motd` with a KDL specification and hooks it into `interactiveShellInit` for interactive SSH logins, displaying system generation, load averages, memory/swap, disk usage bars, IP addresses, and systemd service statuses with subshell guards. |
